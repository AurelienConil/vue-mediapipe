...
export class DistanceFingerBase extends BaseFeatureExtractor {
    readonly name = 'DistanceFingerBase';
    ...
    private readonly fingerBases: Record<Finger, number> = {
        thumb: 4,    // Base du pouce
        index: 6,    // Base de l'index
        middle: 10,  // Base du majeur
        ring: 14,    // Base de l'annulaire
        pinky: 18    // Base de l'auriculaire
    };
    ...
    extract(frame: MediaPipeFrame): Feature[] {
        ...
        for (const hand of frame.hands) {
            ...
            const thumbTip = hand.landmarks[this.fingerBases.thumb];
            ...
            for (const [fingerName, baseIndex] of Object.entries(this.fingerBases)) {
                ...
                if (prevValue && prevValue.timestamp !== undefined) {
                    ...
                    if (dt > 0) {
                        ...
                        features.push({
                            name: `thumb_to_${fingerName}B_distspeed`,
                            type: 'number',
                            value: speed,
                            parents: this.name,
                            display: 'Graph',
                ...
                features.push({
                    name: `thumb_to_${fingerName}B_dist`,
                    type: 'number',
                    value: distance,
                    parents: this.name,
                    display: 'Graph',
